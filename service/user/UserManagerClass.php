...
class UserManagerClass
{
    ...
    public static function getDepartments($userid = '', $infoType = 1)
    {
        ...
        $searchStr = "select distinct d.id as id,d.name as name,d.code as code,d.parentid as parentid from t_User_Department_Set ud,t_Department d where ud.userid in (" . $whereuserid . ") and ud.departmentid=d.id order by d.levels asc,d.sort asc";
    ...
    public static function getRoles($userid = '', $isId = false, $appid = '')
    {
        $tools = new ToolsClass(0);
        $whereuserid = '\'' . str_replace(',', '\',\'', $userid) . '\'';
        $searchStr = "select distinct r.id as id,r.name as name from t_User_Role_Set ur,t_Role r where ur.userid in (" . $whereuserid . ") and ur.roleid=r.id";
        if ($appid != '') {
