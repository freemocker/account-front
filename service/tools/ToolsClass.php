            ...
            if (isset($sqlArray) && $sqlArray != null && count($sqlArray) > 0) {
                ...
                switch ($dbtype) {
                    case 'mysql':
                        ...
                        (select ' . $pKey . ' as q_key_id from ' . $sqlArray[1] . ' ' . $sqlArray[2] . ' ' . $sqlArray[3] . ' limit ' . (($curr_page - 1) * $max_count) . ',' . $max . ')
                        ...
                        break;
                    case 'mssql':
                        $result = $this->connection->doQuery('select top ' . $max_count . ' * from (' . $query_string . ') q_table where q_table.' . $pKey . '
                        not in (select top ' . (($curr_page - 1) * $max_count) . ' q_table_t.' . $pKey . ' from (' . $query_string . ') q_table_t)')->fetchAll();
                        if ($result != null) {
                            $query_result = array(
                                $totalpage,
                                $totalrecord,
