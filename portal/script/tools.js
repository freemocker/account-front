(function ($) {
    ...
    $(window).resize(function () {
        portal_tools_obj.updateTabContentTop();
    });
    ...
    portal_tools_obj = function () {

        portal_tools_obj.mainPageDefault = {
            path: G_homepage_url,
        ...
        this.updateBreadcrumb = function (menuid, title, isAppend, breadcrumbId) {
            var breadcrumbid = breadcrumbId || portal_tools_obj.mainPageDefault.main_breadcrumb_id;
            ...
            if ($breadcrumb.length > 0) {
                if (menuid && menuid != "") {
                    ...
                    portal_tools_obj.doAjax(G_webrootPath + "/service/main/getBreadcrumb", {
                        id: id
                    }, function (data) {
                    ...
                    if (title && title != "") {
                        ...
                        portal_tools_obj.appendBreadcrumb(title, breadcrumbid);
                    } else {
                        $breadcrumb.children(".active").removeClass("active");
                        $breadcrumb.children("li:gt(0)").remove();
                        $breadcrumb.children().eq(0).addClass("active");
        ...
        this.updateTabsInfo = function (menuid, title, container_id, breadcrumbId) {
            ...
            /** 生成选项卡页面div */
            var pageid = "";
            var containerid = container_id || portal_tools_obj.mainPageDefault.container_id;
            ...
            ul.find("a[href='#" + pageid + "']").unbind("click").bind("click", function () {
                if (!ul.find("a[href='#" + pageid + "']").parent().hasClass("active")) {
                    ...
                    portal_tools_obj.setMenuActive(menuid);
                    setTimeout(function () {
            ...
            portal_tools_obj.updateTabContentTop(container_id, breadcrumbId);
            return pageid;
        };
         ...
         */
        portal_tools_obj.setMenuActive = function (menuid, main_nav_id) {
            var main_nav_id = main_nav_id || portal_tools_obj.mainPageDefault.main_nav_id;
     ...
     * 退出登录
     */
    portal_tools_obj.doLogout = function () {
     ...
     * 登录超时调用页面
     */
    portal_tools_obj.loginTimeout = function () {
     ...
     */
    portal_tools_obj.doAjax = function (url, param, succesFun, type, isShowProcess, dataType, async, errorFun, timeOut) {
     ...
     *            可选。调用失败时执行的函数 function(obj, message, exception)
     */
    portal_tools_obj.doAjaxToServer = function (serverno, head, body, succesFun, isShowProcess, dataType, errorFun) {
        var head = $.extend(head, {serverno: serverno});
        var body = $.extend(body, {});
        _global_tools_obj.doAjaxToServer(G_webrootPath + "/view/common/communicationToBack", G_backService.httphostIn, G_backService.charset, {
            head: head,
            body: body
        }, succesFun, G_backService.httptimeout, isShowProcess, dataType, errorFun, "json");
    };
     ...
     *            可选。调用失败时执行的函数 function(obj, message, exception)
     */
    portal_tools_obj.doAjaxToServer2 = function (serverno, head, body, succesFun, isShowProcess, dataType, errorFun) {
        var head = $.extend(head, {serverno: serverno});
        var body = $.extend(body, {});
        _global_tools_obj.doAjaxToServer(G_webrootPath + "/view/common/communicationToBack2", G_backService2.httphostIn, G_backService2.charset, {
            head: head,
            body: body
        }, succesFun, G_backService2.httptimeout, isShowProcess, dataType, errorFun, "json");
    };
     ...
     *            菜单全路径定位，“/”分隔
     */
    portal_tools_obj.gotoMenuPage = function (menuePath) {
        var top = window.top;
        if (!menuePath || menuePath == "") {
            AUI.dialog.alert("找不到页面或没有权限访问！【" + menuePath + "】", null, 3);
        } else {
            portal_tools_obj.doAjax(G_webrootPath + "/service/main/gotoMenuPage", {
                menuePath: menuePath
            }, function (menuid) {
     ...
     * @param breadcrumbId
     */
    portal_tools_obj.appendBreadcrumb = function (currName, breadcrumbId) {
     ...
     * @param breadcrumbId
     */
    portal_tools_obj.doBackBreadcrumb = function (breadcrumbId) {
        if (!G_settings_use_tabs) {
            var breadcrumbid = breadcrumbId || portal_tools_obj.mainPageDefault.main_breadcrumb_id;
            var $breadcrumb = window.top.$("#" + breadcrumbid);
            if ($breadcrumb.length > 0) {
     ...
     *            是否进行加密下载
     */
    portal_tools_obj.doDownloadFromBack = function (filename, isdelete, issec) {
        var backhost = G_backService.httphostIn;
        backhost = backhost.substring(0, backhost.lastIndexOf("/") + 1);
        _tools_file_obj.doDownloadFromBack(G_webrootPath + "/view/common/communicationToBack", backhost, G_backService.charset, filename, isdelete, issec);
    };
     ...
     *            是否进行加密下载
     */
    portal_tools_obj.doDownloadFromBack2 = function (filename, isdelete, issec) {
        var backhost = G_backService2.httphostIn;
        backhost = backhost.substring(0, backhost.lastIndexOf("/") + 1);
        _tools_file_obj.doDownloadFromBack(G_webrootPath + "/view/common/communicationToBack2", backhost, G_backService2.charset, filename, isdelete, issec);
    };
     ...
     */
    portal_tools_obj.loadPageInDiv = function (param) {
        var thisobj = new portal_tools_obj();
        var param = $.extend(portal_tools_obj.mainPageDefault, param);

        /** 更新菜单选中状态 */
        portal_tools_obj.setMenuActive(param.menuid, param.main_nav_id);
    ...
    portal_tools_obj.showAuthDialog = function (callBack) {
        AUI.dialog.inDialog(400, 250, "授权", {
            innerUrl: G_webrootPath + "/view/page/tradepower/tradepower"
        }, null, function (data) {
            if (data) {
                if (data.authloginno && data.authpassword) {
                    var str1 = _tools_security_obj.encryptMD5(data.authpassword).toLowerCase();
                    var str2 = _tools_security_obj.encryptMD5(str1 + data.authloginno).toLowerCase();
                    var authdata = {};
                    authdata.authloginno = data.authloginno;
                    authdata.authpassword = str2;
     ...
     * @param breadcrumbId
     */
    portal_tools_obj.updateTabContentTop = function (container_id, breadcrumbId) {
        var containerid = container_id || portal_tools_obj.mainPageDefault.container_id;
        var main_breadcrumb_id = breadcrumbId || portal_tools_obj.mainPageDefault.main_breadcrumb_id;
    ...
    function closeThisTab(obj, container_id) {
        ...
        portal_tools_obj.updateTabContentTop(container_id, ul.attr("id"));
    }
    ...
    function closeOtherDropdownMenu(obj, main_breadcrumb_id) {
        ...
        var main_breadcrumb_id = main_breadcrumb_id || portal_tools_obj.mainPageDefault.main_breadcrumb_id;
    ...
    $.extend(true, $.jgrid.inlineEdit, {
        beforeSaveRow: function (obj, rowid) {
            var customerParams = AUI.grid.getGridCustomerParams(this);
            if (customerParams) {
                if (customerParams.needAuth) {
                    if ($.trim($("#" + rowid + "_authloginno").val()) != "" && $.trim($("#" + rowid + "_authpassword").val()) != "") {
                        return true;
                    } else {
                        portal_tools_obj.showAuthDialog(function (authdata) {
                            $("#" + rowid + "_authloginno").val(authdata.authloginno);
                            $("#" + rowid + "_authpassword").val(authdata.authpassword);
                            $("#jSaveButton_" + rowid).click();
                        });
                    }
                    return false;
                }
        ...
        serializeDelData: function (postdata) {
            ...
            if (customerParams) {
                if (customerParams.needAuth) {
                    ...
                    if (authdata && authdata.authloginno && authdata.authpassword) {
                        $this.removeData("authdata");
                        postdata.authloginno = authdata.authloginno;
                        postdata.authpassword = authdata.authpassword;
                    } else {
                        portal_tools_obj.showAuthDialog(function (authdata) {
