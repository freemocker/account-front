...
namespace portal\service\login;
    ...
    public function doLogin($username, $password, $yzm)
    {
        ...
        try {
            if (isset($_SESSION[ToolsClass::$LOGIN_YZM_STR])) {
                ...
                if (strtolower($yzm) != $session_yzm) {
                    $result[0] = 1;
                    $result[1] = "验证码输入不正确！";
                } else {
                    $result = UserManagerClass::doLogin($username, $password, $session_yzm, ToolsClass::$LOGIN_USER_STR, $GLOBALS['application'], PortalConfig::getInstance()['singlePoint']);
                }
            } else {
                $result[0] = 3;
                $result[1] = "登录失败：非法登录";
                LoggerClass::error('[应用:' . PortalConfig::getInstance()['webroot'] . ']' . '非法登录');
            }
        } catch (\Exception $e) {
            $result[0] = 3;
            $result[1] = "登录失败：" . $e->getMessage();
            LoggerClass::error_e('[应用:' . PortalConfig::getInstance()['webroot'] . ']' . $e->getMessage(), $e);
        }
        return $result;
    }
