...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['oper'])) {
                    ...
                    if ($countnumber <= 0) {
                        ...
                        if (isset($_POST['roleids']) && $_POST['roleids'] != '') {
                            foreach ($_POST['roleids'] as $roleid) {
                                $connection->addBatch("insert into T_user_role_set(id,userid,roleid) values('" . \service\tools\common\UUIDClass::getUUID() . "','" . $userid . "','" . $roleid . "')");
                            }
                        }
                        if (isset($_POST['departmentids']) && $_POST['departmentids'] != '') {
                            ...
                            if (validateDepPower($highRoleLevel, $departmentids)) {
                                foreach ($departmentids as $departmentid) {
                                    $connection->addBatch("insert into T_user_department_set(id,userid,departmentid) values('" . \service\tools\common\UUIDClass::getUUID() . "','" . $userid . "','" . $departmentid . "')");
                                }
                            } else {
            ...
            case "del":
                ...
                foreach ($userlist as $user) {
                    ...
                    if ($user['level'] > $currLevel) {
                        $connection->addBatch("delete from oms_sysuser_token where userid='" . $user['id'] . "'");
            ...
            case "rpw":
                ...
                if ($level > $currLevel) {
                    if (\service\user\UserManagerClass::resetPassword($_POST['id'], $userinfo['loginno'], $adminconfig['defaultPassword'])) {
                        $connection->doExcute("delete from oms_sysuser_token where userid='" . $_POST['id'] . "'");
                        $result['result'] = "重置成功！";
                    } else {
        ...
        $sqlArray[1] = $sqlArray[1] . " order by u.levels asc,u.sort asc)t";
