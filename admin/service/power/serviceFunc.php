    ...
    if (isset($_POST['cmd'])) {
        ...
        switch ($cmd) {
                ...
                if (validateCode(true, $moduleid, $_POST['module_code'])) {
                    ...
                    if (!empty($_POST['select_roles']) && $_POST['select_roles'] != "") {
                        ...
                        foreach ($select_roles as $roleid) {
                            $connection->addBatch("insert into t_Role_Module_Set(id,roleid,moduleid) values('" . service\tools\common\UUIDClass::getUUID() . "','" . $roleid . "','" . $moduleid . "')");
                        }
                    }
                    if (!$connection->doExecBatch()) {
            ...
            case "searchFunc":
                ...
                $sqlArray = array();
                $sqlArray[0] = "*";
                $sqlArray[1] = "(select id,appid as applicationid,name,code,case islog when 1 then '是' else '否' end as islog from t_Module_Func where moduleid='" . $moduleid . "' and appid='" . $_POST['appid'] . "' order by code asc)t";
            ...
            case "saveFuncRoles":
                ...
                if (!empty($_POST['select_roles']) && $_POST['select_roles'] != "") {
                    $select_roles = $_POST['select_roles'];
                    foreach ($select_roles as $roleid) {
                        $connection->addBatch("insert into t_Role_Module_Func_Set(id,roleid,funcid) values('" . service\tools\common\UUIDClass::getUUID() . "','" . $roleid . "','" . $_POST['funcid'] . "')");
                    }
                }
                if (!$connection->doExecBatch()) {
                    $result = \service\tools\ToolsClass::buildJSONError("保存信息失败！");
