...
$topmenus = array(); // 顶层菜单
$tools = new admin\service\tools\ToolsClass(0);
...
<!-- 导航 start-->
<div
        class="navbar navbar-default<?php if ($GLOBALS['settings_navbar'] == 1) {
            echo " navbar-fixed-top";
        } ?>">
    <div class="navbar-container">
        <!-- 小分辨率菜单按钮 start-->
        <button type="button" class="navbar-toggle menu-toggler pull-left"
                data-target="#main_page_sidebar">
        ...
        <div class="navbar-buttons navbar-header pull-right"
            ...
            <ul class="nav ace-nav">
                <li style="margin-right: 20px;"><span id="main_now_time"
                                                      class="white"></span></li>
                <li class="light-blue"><a data-toggle="dropdown"
                                          href="javascript:void(0)" class="dropdown-toggle"> <img
                                class="nav-user-photo" style="max-height: 40px;"
                                src="
<?php
                                ...
                                ?>"
                                alt="头像"/> <span class="user-info"
                                                 title="<?php if ($departments !== '') {
                                                     echo $departments;
                                                     echo '（' . $user->getName() . '）';
                                                 } else {
                                                     echo $user->getName();
                    ...
                    <ul
                        ...
                        <li><a href="javascript:void(0)" id="main_setting_a"> <i
                                        class="ace-icon fa fa-cog"></i> 个性设置
                            </a></li>

                        <li><a href="javascript:void(0)" id="main_user_info_a"> <i
                                        class="ace-icon fa fa-user"></i> 个人资料
                            </a></li>

                        <li class="divider"></li>

                        <li><a href="javascript:void(0)" id="main_loginout_a"> <i
...
<!-- 页面区域 start -->
<div
        class="main-container<?php if ($GLOBALS['settings_add_container'] == 1) {
            echo " container";
        } ?>">
    <!-- 侧边栏 start -->
    <div id="main_page_sidebar"
         class="sidebar responsive<?php if ($GLOBALS['settings_sidebar'] == 1) {
             echo " sidebar-fixed";
         }
        ...
        if ($sysparamconfig || $appconfig || $roleconfig || $powerconfig) {
            ?>
            <div class="sidebar-shortcuts">
                <!-- 大按钮 -->
                <div class="sidebar-shortcuts-large">
                    <?php if ($user->getLevels() <= 0 || $sysparamconfig) { ?>
                        <button type="button" class="btn btn-danger"
                                id="main_sysparam_conf_btn" title="系统参数">
                            <i class="ace-icon fa fa-inbox"></i>
                        </button>
                    <?php } ?>
                    <?php if ($user->getLevels() <= 0 || $appconfig) { ?>
                        <button type="button" class="btn btn-success"
                                id="main_app_conf_btn" title="应用配置">
                            <i class="ace-icon fa fa-desktop"></i>
                        </button>
                    <?php } ?>
                    <?php if ($user->getLevels() <= 0 || $roleconfig) { ?>
                        <button type="button" class="btn btn-warning"
                                id="main_role_conf_btn" title="角色配置">
                            <i class="ace-icon fa fa-user-md"></i>
                        </button>
                    <?php } ?>
                    <?php if ($user->getLevels() <= 0 || $powerconfig) { ?>
                        <button type="button" class="btn btn-info" id="main_power_conf_btn"
                                title="权限配置">
    ...
    <div class="main-content">
        ...
        <div class="main-content-inner">
            <?php if ($GLOBALS['settings_use_tabs'] !== 1) { ?>
                <!-- 顶部面板 start -->
                <div
                        class="breadcrumbs<?php if ($GLOBALS['settings_breadcrumbs'] == 1) {
                            echo " breadcrumbs-fixed";
                        } ?>">
                    <!-- 页面路径 start -->
                    <ul class="breadcrumb" id="main_breadcrumb">
                        <li class="active"><i class="ace-icon fa fa-home home-icon"></i> <a
                                    href="javascript:admin_tools_obj.loadPageInDiv();" class="main_menu">首页</a></li>
                    </ul>
                    <!-- 页面路径 end -->
                    <!-- 顶部面板 end -->
                </div>
            ...
            <?php } ?>
            <!-- 页面内容 start -->
            <div id="main_page_content_div"
                 class="page-content main-page-container tab-content main-tabbable-content">
            </div>
            <!-- 页面内容 end -->
        </div>
    </div>
...
<script type="text/javascript" defer async="async"
        ...
        src="../script/main/main.js<?php echo '?v=1.0.0'; ?>"></script>
